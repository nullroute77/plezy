...
void main() {
  ...
  setUpAll(() async {
    ...
    if (_screenshotDir.isNotEmpty) {
      await (FontLoader('BadgePreview')..addFont(rootBundle.load('assets/go-noto-current-regular.ttf'))).load();
      await (FontLoader(
  ...
  for (final scale in [1.0, 1.5, 2.0]) {
    testWidgets('shared LIVE badge keeps symmetric bounds with text scale $scale', (tester) async {
      await tester.pumpWidget(
        _shell(
          MediaQuery(
